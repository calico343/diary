...
(function($) {
   ...
   $.widget("ui.weekCalendar", {
       ...
       options : {
         ...
         timeslotsPerHour 			: 4,
         buttons 					: true,
         buttonText 				: { today : "Today", lastWeek : "&nbsp;&lt Previous &nbsp;", nextWeek : "&nbsp;Next &gt;&nbsp;" },
      ...
      prevWeek : function() 
      {
         ...
         $('#datehold').val(this.element.data("startDate")); 
         this._clearCalendar();
...
/*
      ...
      */
      ...
      _renderCalendar : function() {
         ...
         if (options.buttons) {
            calendarNavHtml = "<div class=\"wc-nav\"><div class = \"wc-left\">Calendar - Week View</div>\
                    <button class=\"wc-today calbutton\">" + options.buttonText.today + "</button>\
                    <button class=\"wc-month calbutton\"> Month View </button>\
                    <button class=\"wc-prev calbutton\">" + options.buttonText.lastWeek + "</button>\
                    <button class=\"wc-next calbutton\">" + options.buttonText.nextWeek + "</button>\
                    </div>";
            ...
            $calendarContainer.find(".wc-nav .wc-date").click(function() {
                self.element.weekCalendar("gotoWeek");
                return false;
             });
         ...
         for (var i = 1; i <= options.daysToShow; i++) 
         {
        	 calendarBodyHtml += "<td id = 'dwref-" + i + "' class=\"wc-day-column day-" + i + "\"><div class=\"wc-day-column-inner\" ></div></td>";
         }
      ...
      _renderEvent: function (calEvent, $weekDay) 
      {
         ...
         var eventDirectStyle, eventClass, eventHtml, $calEvent, $modifiedEvent, fontcolor, bordercolor;

         calEvent.site == "RFH" ? bordercolor = '#0B20DE': bordercolor = '#961E1E'; 
        
         fontcolor			= parseInt(calEvent.colcode, 16) > ((0xffffff/2) - 9000000) ? '#000000':'#FFFFFF';
         eventDirectStyle	= " style = 'background-color:#" + calEvent.colcode + ";color:" + fontcolor + "' ";
         eventClass 		= calEvent.id ? "wc-cal-event " : "wc-cal-event wc-new-cal-event ";
         eventHtml 			= "<div style = 'background-color:#CCCCCC;border: 2pt solid "+ bordercolor +"' class=\"" + eventClass + " ui-corner-all\">\
                				<div" + eventDirectStyle + " class=\"wc-time ui-corner-all " + calEvent.CssClass + "\"></div>\
                				<div class=\"wc-title " + calEvent.site + "\"></div></div>";
      ...
      _refreshEventDetails : function(calEvent, $calEvent) 
      {
         ...
         {
        	 $calEvent.find(".wc-title").html(self._formatDate(calEvent.start, options.timeFormat) + ": " + calEvent.title);
         }
         else 
         {
